...
\subsection{内容梗概}
围绕快速多极算法的文章主要描述了一种基于多极展开的二维快速多级算法，用于解决上述N 体问题。其主要思想是通过对远场域粒子产生的势进行展开并截断以获取给定精度下的逼近式，来避免直接计算粒子间的两两相互作用。在运算过程中，通过将所求区域不断细分来获得各个区域的远场域展开式。由于计算过程中基本都只涉及对展开式的处理，因此只需存储与计算p个展开式系数即可，由于p 相对于N而言为常数，因此大大提高了运算效率与存储效率。
...
\bibitem{1}Gareengard L , Rokhlin V . A fast algorithm for particle simulations[J]. Journal of Computational Physics, 1997, 73(2):325-348.
\bibitem{2}Leslie G . The Numerical Solution of the N-Body Problem[J]. Computers in Physics, 1990, 4(2):142-152.
\bibitem{3}R.K. Beatson, L. Greengard. A short course on fast multipole methods[M]. Oxford University Press, 1997,1~37
\bibitem{4}弓晓东，胡俊，聂在平等. 三维导体目标电磁散射的高阶多层快速多极子方法[J]. 电波科学学报，2004,19(5):557-580.
\bibitem{5}胡俊，聂在平，王军等. 三维电大目标散射求解的多层快速多极子方法[J]. 电波科学学报，2004,19(5):509-514.
\bibitem{6}卢光辉，孙世新，聂在平等.并行处理技术在电大尺寸复杂目标电磁散射中的应用[J]. 电子学报,2003,31(6):882~885
\bibitem{7}Cui T J, Chew W C, Chen G, et al. Efficietn MLFMA, RPFMA, and FAFFA algorithms for EM scattering by very large structures[J]. IEEE Transactions on antennas and Propagation，2004,52(3):759-770.
\bibitem{8}Song J M, Chew W C. Multilevel fast-multipole algorithm for solving combined field integral equations of electromagnetic scattering[J]. Microwave and Optical Technology Letters, 1995, 10(1): 14-19.
\bibitem{9}Lu C C, Chew W C. A multilevel algorithm for solving a boundary integral equation of wave scattering[J].Microwave and Optical Technology Letters, 1994, 7(10):466-470.
\bibitem{10}Song J M, Chew W C. Error analysis for the truncation of multipole expansion of vector Green’s functions[J].Microwave and Wireless Components Letters, 2001, 11(7):311-312.
\end{thebibliography}
