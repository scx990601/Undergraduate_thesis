...
box_geom_son = [1,0.8,0.6];
xx_son       = [box_geom_son(2) + box_geom_son(1)*(rand(1,200)-0.5);...
                box_geom_son(3) + box_geom_son(1)*(rand(1,200)-0.5)];
box_geom_dad = [2,  1,  1];
xx_outer_n   = [box_geom_dad(2) + 3.0*box_geom_dad(1)*(rand(1,100)-0.5);...
                box_geom_dad(3) + 1.5*box_geom_dad(1)*ones(1,100)];
xx_outer_s   = [box_geom_dad(2) + 3.0*box_geom_dad(1)*(rand(1,100)-0.5);...
                box_geom_dad(3) - 1.5*box_geom_dad(1)*ones(1,100)];
xx_outer_w   = [box_geom_dad(2) - 1.5*box_geom_dad(1)*ones(1,100);...
                box_geom_dad(3) + 3.0*box_geom_dad(1)*(rand(1,100)-0.5)];
xx_outer_e   = [box_geom_dad(2) + 1.5*box_geom_dad(1)*ones(1,100);...
                box_geom_dad(3) + 3.0*box_geom_dad(1)*(rand(1,100)-0.5)];
xx_outer     = [xx_outer_n,xx_outer_w,xx_outer_s,xx_outer_e];

fprintf(1,'Testing out-to-out and in-to-in.\n')
A     = LOCAL_A_offd(xx_outer,xx_son);
T_ofs = LOCAL_T_ofs(box_geom_son,p_fmm,xx_son);
T_ofo = LOCAL_T_ofo(box_geom_dad,box_geom_son,p_fmm);
T_tfo = LOCAL_T_tfo(xx_outer,box_geom_dad,p_fmm);
ERR = A - real(T_tfo * T_ofo * T_ofs);
fprintf(1,'ERR = A - T_tfo * T_ofo * T_ofs :    %12.5e      %12.5e      %12.5e\n',norm(ERR),norm(ERR)/norm(A),max(max(abs(ERR))))
A     = LOCAL_A_offd(xx_son,xx_outer);
T_ifs = LOCAL_T_ifs(box_geom_dad,p_fmm,xx_outer);
T_ifi = LOCAL_T_ifi(box_geom_son,box_geom_dad,p_fmm);
T_tfi = LOCAL_T_tfi(xx_son,box_geom_son,p_fmm);
...
axis equal
figure(2)
plot(box_geom_son(2) + box_geom_son(1)*[-0.5,0.5,0.5,-0.5,-0.5],...
     box_geom_son(3) + box_geom_son(1)*[-0.5,-0.5,0.5,0.5,-0.5],'-r',...
     xx_son(1,:),xx_son(2,:),'r.',...
     box_geom_dad(2) + box_geom_dad(1)*[-0.5,0.5,0.5,-0.5,-0.5],...
     box_geom_dad(3) + box_geom_dad(1)*[-0.5,-0.5,0.5,0.5,-0.5],'-b',...
     xx_outer(1,:),xx_outer(2,:),'k.')
legend('son','xxson','dad','xxouter')
axis equal
