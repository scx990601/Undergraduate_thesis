% This program visualizes every step of an an execution of the FMM.
%
% After each step, it draws a figure and invokes a "keyboard" command.
%
% To move to the next step, simply type 
%
%  K>> return
%
% When you want to visualize the next "type" of interaction (in other 
% words, the next "interaction list), type:
%
%  K>> flag_plot = 0;
%  K>> return

function main_simplefmm_visualize
...
%%%%%%%%%%%%%%% Set the geometry.
flag_geo = 2;   % Set this parameter to 1/2/3 to choose different particle distributions.
if (flag_geo == 1)
  nmax = 25;
  ntot = 600;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
qq = randn(ntot,3);
uu = LOCAL_FMM(xx,qq,nmax,p_fmm,box_geom_root);
...
function uu_orig = LOCAL_FMM(xx_orig,qq_orig,nmax,p_fmm,box_geom_root)
...
xx             = xx_orig(:,indvec);
figure(1)
LOCAL_draw_boxes_row(NODES,xx,box_geom_root);
...
for ibox = nboxes:(-1):2
  if (length(NODES{04,ibox}) == 0) % ibox has no children
    ...
    flag_plot = LOCAL_plot_ofs(NODES,xx,ibox,flag_plot);
  end
end
...
flag_plot = 1;
for ibox = nboxes:(-1):2
  if ((NODES{2,ibox} > 1) &&(length(NODES{04,ibox}) > 0))
    ...
    flag_plot = LOCAL_plot_ofo(NODES,ibox,flag_plot);
  end
end
...
for ibox = 1:nboxes
  if (length(NODES{12,ibox}) > 0)
    ...
    flag_plot = LOCAL_plot_ifo(NODES,ibox,flag_plot);
  end
end
...
for ibox = 1:nboxes
  if (length(NODES{14,ibox}) > 0)
    ...
    flag_plot = LOCAL_plot_tfo(NODES,xx,ibox,flag_plot);
  end
end
...
for ibox = 1:nboxes
  if (length(NODES{16,ibox}) > 0)
    ...
    flag_plot = LOCAL_plot_ifs(NODES,xx,ibox,flag_plot);
  end
end
...
for ibox = 2:nboxes
  if ((NODES{2,ibox} > 1) && (length(NODES{04,ibox}) > 0))
    ...
    flag_plot = LOCAL_plot_ifi(NODES,ibox,flag_plot);
  end
end
...
for ibox = 2:nboxes
  if (length(NODES{04,ibox}) == 0) % ibox is a leaf
    ...
    flag_plot = LOCAL_plot_tfi(NODES,xx,ibox,flag_plot);
  end
end
...
for ibox = 1:nboxes
  if (length(NODES{10,ibox}) > 0)
    ...
    flag_plot = LOCAL_plot_close(NODES,xx,ibox,flag_plot);
  end
end
...
function ans = LOCAL_draw_boxes(NODES,xx,box_geom)
...
if (flag_warning == 1)
  disp(sprintf('WARNING in FMM01_draw_boxes: More than 4 levels!!'))
end
...
function [X1_LEAF,X2_LEAF] = LOCAL_get_XX_LEAF(NODES)

nboxes = size(NODES,2);

X1_LEAF = zeros(5,nboxes);
X2_LEAF = zeros(5,nboxes);
...
for ibox = 2:nboxes 
  if isempty(NODES{4,ibox})
    nleaf = nleaf + 1;
    box   = NODES{1,ibox};
    X1_LEAF(:,nleaf) = box(2) + 0.5*box(1)*[-1; 1; 1;-1;-1];
    X2_LEAF(:,nleaf) = box(3) + 0.5*box(1)*[-1;-1; 1;1;-1];
  end
end

X1_LEAF = X1_LEAF(:,1:nleaf);
X2_LEAF = X2_LEAF(:,1:nleaf);

return

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function flag_plot = LOCAL_plot_ofs(NODES,xx,ibox,flag_plot)

if (flag_plot == 0)
  return
end

[X1_LEAF,X2_LEAF] = LOCAL_get_XX_LEAF(NODES);
...
figure(2)
hold off
plot(X1_LEAF,X2_LEAF,'k')
hold on
h = plot(box_geom(2) + 0.5*box_geom(1)*[-1, 1, 1,-1,-1],...
         box_geom(3) + 0.5*box_geom(1)*[-1,-1, 1, 1,-1],'g');
set(h,'LineWidth',3)
plot([xx(1,ind);box_geom(2)*ones(1,length(ind))],...
     [xx(2,ind);box_geom(3)*ones(1,length(ind))],'k')
hc = plot(box_geom(2),box_geom(3),'r.');
set(hc,'MarkerSize',20);
...
function flag_plot = LOCAL_plot_ofo(NODES,ibox,flag_plot)
...
box_geom_dad = NODES{1,ibox};

figure(2)
hold off
plot(X1_LEAF,X2_LEAF,'k')
hold on
h = plot(box_geom_dad(2) + 0.5*box_geom_dad(1)*[-1, 1, 1,-1,-1],...
         box_geom_dad(3) + 0.5*box_geom_dad(1)*[-1,-1, 1, 1,-1],'g');
set(h,'LineWidth',3)
for ison = NODES{04,ibox}
  box_geom_son   = NODES{01,ison};
  h = plot(box_geom_son(2) + 0.5*box_geom_son(1)*[-1, 1, 1,-1,-1],...
           box_geom_son(3) + 0.5*box_geom_son(1)*[-1,-1, 1, 1,-1],'r',...
           [box_geom_son(2),box_geom_dad(2)],...
           [box_geom_son(3),box_geom_dad(3)],'k',...
           [box_geom_son(2),box_geom_dad(2)],...
          [box_geom_son(3),box_geom_dad(3)],'r.');
...
function flag_plot = LOCAL_plot_ifo(NODES,ibox,flag_plot)
...
plot(X1_LEAF,X2_LEAF,'k')
hold on
h = plot(box_geom_trg(2) + 0.5*box_geom_trg(1)*[-1, 1, 1,-1,-1],...
         box_geom_trg(3) + 0.5*box_geom_trg(1)*[-1,-1, 1, 1,-1],'g');
set(h,'LineWidth',3)
for jbox = NODES{12,ibox}
   box_geom_src   = NODES{01,jbox};
   h = plot(box_geom_src(2) + 0.5*box_geom_src(1)*[-1, 1, 1,-1,-1],...
            box_geom_src(3) + 0.5*box_geom_src(1)*[-1,-1, 1, 1,-1],'r',...
            [box_geom_src(2),box_geom_trg(2)],...
            [box_geom_src(3),box_geom_trg(3)],'k',...
             box_geom_src(2),box_geom_src(3),'r.',...
             box_geom_trg(2),box_geom_trg(3),'b.');
   set(h(3),'MarkerSize',30)
...
function flag_plot = LOCAL_plot_tfo(NODES,xx,ibox,flag_plot)

if (flag_plot == 0)
  return
end
[X1_LEAF,X2_LEAF] = LOCAL_get_XX_LEAF(NODES);
J_trg = NODES{6,ibox} - 1 + (1:NODES{7,ibox});
...
h = plot(box_geom_trg(2) + 0.5*box_geom_trg(1)*[-1, 1, 1,-1,-1],...
         box_geom_trg(3) + 0.5*box_geom_trg(1)*[-1,-1, 1, 1,-1],'g',...
         xx(1,J_trg),xx(2,J_trg),'b.');
set(h(1),'LineWidth',3)
for jbox = NODES{14,ibox}
  box_geom_src   = NODES{01,jbox};
  plot(box_geom_src(2) + 0.5*box_geom_src(1)*[-1, 1, 1,-1,-1],...
       box_geom_src(3) + 0.5*box_geom_src(1)*[-1,-1, 1, 1,-1],'r',...
       [box_geom_src(2)*ones(1,length(J_trg));xx(1,J_trg)],...
       [box_geom_src(3)*ones(1,length(J_trg));xx(2,J_trg)],'k')
  h = plot(box_geom_src(2),box_geom_src(3),'r.');
  set(h,'MarkerSize',30)
end
axis equal
axis off
...
function flag_plot = LOCAL_plot_ifs(NODES,xx,ibox,flag_plot)
...
h = plot(box_geom_trg(2) + 0.5*box_geom_trg(1)*[-1, 1, 1,-1,-1],...
         box_geom_trg(3) + 0.5*box_geom_trg(1)*[-1,-1, 1, 1,-1],'g');
set(h,'LineWidth',3)
for jbox = NODES{16,ibox}
  box_geom_src   = NODES{01,jbox};
  J_src         = NODES{06,jbox} - 1 + (1:NODES{07,jbox});
  plot(box_geom_src(2) + 0.5*box_geom_src(1)*[-1, 1, 1,-1,-1],...
       box_geom_src(3) + 0.5*box_geom_src(1)*[-1,-1, 1, 1,-1],'r',...
       [box_geom_trg(2)*ones(1,length(J_src));xx(1,J_src)],...
       [box_geom_trg(3)*ones(1,length(J_src));xx(2,J_src)],'k')
  h = plot(box_geom_trg(2),box_geom_trg(3),'b.');
  set(h,'MarkerSize',30)
  plot(xx(1,J_src),xx(2,J_src),'r.')
end
axis equal
axis off
hold off
...
function flag_plot = LOCAL_plot_ifi(NODES,ibox,flag_plot)
...
for ison = NODES{04,ibox}
  box_geom_son   = NODES{01,ison};
  h = plot(box_geom_son(2) + 0.5*box_geom_son(1)*[-1, 1, 1,-1,-1],...
           box_geom_son(3) + 0.5*box_geom_son(1)*[-1,-1, 1, 1,-1],'b',...
           [box_geom_son(2),box_geom_dad(2)],...
           [box_geom_son(3),box_geom_dad(3)],'k',...
           [box_geom_son(2),box_geom_dad(2)],...
           [box_geom_son(3),box_geom_dad(3)],'b.');
  set(h(3),'MarkerSize',30)
end
axis equal
axis off
...
function flag_plot = LOCAL_plot_tfi(NODES,xx,ibox,flag_plot)
...
plot([xx(1,ind);box_geom(2)*ones(1,length(ind))],...
     [xx(2,ind);box_geom(3)*ones(1,length(ind))],'k')
hc = plot(box_geom(2),box_geom(3),'b.');
set(hc,'MarkerSize',20);
plot(xx(1,ind),xx(2,ind),'b.')
axis equal
axis off
hold off
keyboard
...
function flag_plot = LOCAL_plot_close(NODES,xx,ibox,flag_plot)
...
function ans = LOCAL_draw_boxes_row(NODES,xx,box_geom)
